...
interface Booking {
  ...
  totalPrice: number;
  pricingSnapshot?: string | null; // Phase 2: Canonical pricing snapshot (JSON string)
...
function BookingsPageContent() {
  ...
  // Phase 6.2: Today board states
  const [todayBoardData, setTodayBoardData] = useState<any>(null);
  const [loadingTodayBoard, setLoadingTodayBoard] = useState(false);
  ...
  // Phase 6.2: Fetch Today board data
  const fetchTodayBoard = async () => {
    setLoadingTodayBoard(true);
    try {
      const response = await fetch("/api/bookings/today");
      if (!response.ok) {
        throw new Error(`Failed to fetch today board: ${response.status}`);
      }
      const data = await response.json();
      setTodayBoardData(data);
    } catch (error) {
      console.error("Failed to fetch today board:", error);
      setTodayBoardData(null);
    } finally {
      setLoadingTodayBoard(false);
    }
  };
  ...
  const filteredBookings = useMemo(() => {
    ...
    // Filter by status (skip if "today" - handled separately)
  ...
  return (
    <div className="min-h-screen w-full" style={{ background: COLORS.primaryLighter }}>
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 sm:gap-5">
            ...
            <div className="relative w-full">
              ...
              <Link
                ...
                style={{ color: COLORS.primary, borderColor: COLORS.primaryLight }}
              >
                  <i className="fas fa-exclamation-triangle"></i><span>Exceptions</span>
              </Link>
              <Link
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-6 sm:space-y-8">
        ...
        <div className="bg-white rounded-lg p-3 sm:p-4 border-2 mb-4 sm:mb-6" style={{ borderColor: COLORS.primaryLight }}>
          <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 w-full sm:w-auto">
              ...
              <select
                value={filter}
                onChange={(e) => setFilter(e.target.value as "all" | "pending" | "confirmed" | "completed" | "cancelled" | "today")}
                className="px-3 py-2 sm:py-1 border rounded-lg text-sm w-full sm:w-auto"
                style={{ borderColor: COLORS.border }}
              >
                <option value="all">All</option>
                <option value="today">Today Board</option>
        ...
        {/* Today Board View (Phase 6.2) */}
        {filter === "today" && todayBoardData && !loadingTodayBoard && (
          <div className="bg-white rounded-lg border-2 p-4 sm:p-6" style={{ borderColor: COLORS.primaryLight }}>
        ...
        {filter === "today" && loadingTodayBoard && (
          <div className="bg-white rounded-lg border-2 p-8 text-center" style={{ borderColor: COLORS.primaryLight }}>
            ...
            <p className="text-gray-600">Loading today board...</p>
          </div>
        )}
                        ...
                        <div className="md:col-span-2 space-y-4">
                          ...
                          <div className="bg-gray-50 rounded-lg p-4 border" style={{ borderColor: COLORS.border }}>
                            {(() => {
                              ...
                              let breakdown;
                              try {
                                // Phase 2: In edit mode, always calculate (no snapshot yet). In view mode, use snapshot if available.
                                if (isEditMode) {
                                  // Edit mode: calculate from current edited values (snapshot doesn't exist yet)
                                  breakdown = calculatePriceBreakdown(calcInput as any);
                                } else {
                                  // View mode: use snapshot when flag enabled and available
                                  const pricingDisplay = getPricingForDisplay(calcInput as any);
                                  breakdown = {
                                    total: pricingDisplay.total,
                                    breakdown: pricingDisplay.breakdown,
                                    basePrice: 0,
                                    additionalPets: 0,
                                    holidayAdd: 0,
                                    afterHoursAdd: 0,
                    ...
                    <div className="bg-white border-2 rounded-xl p-6 shadow-sm" style={{ borderColor: COLORS.primaryLight }}>
                      ...
                      <div className="space-y-3">
                        ...
                        <button
                          onClick={() => {
                            if (!selectedBooking) return;
                            // Phase 2: Use pricing display helper (uses snapshot when flag enabled)
                            const pricingDisplay = getPricingForDisplay(selectedBooking);
                            const breakdown = {
                              total: pricingDisplay.total,
                              breakdown: pricingDisplay.breakdown,
                              basePrice: 0, // Not used in display
                              additionalPets: 0,
                              holidayAdd: 0,
                              afterHoursAdd: 0,
                              quantity: selectedBooking.quantity || 1,
                            };
                            const calculatedTotal = pricingDisplay.total;
