        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link href="/client/pets/new">
              <Button variant="primary" size="md" leftIcon={<Plus size={16} />}>Add pet</Button>
            </Link>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {pets.map((pet) => (
              <Link key={pet.id} href={`/client/pets/${pet.id}`}>
                <div className="rounded-xl border border-border-default bg-surface-primary p-5 hover:shadow-[var(--shadow-md)] hover:border-border-strong transition-all duration-200 cursor-pointer">
