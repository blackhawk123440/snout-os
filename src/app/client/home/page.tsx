    ...
    <LayoutWrapper variant="narrow">
      {loading ? (
        ...
        <div className="space-y-4">
          ...
          {/* ─── Next Visit Hero ────────────────────────────────── */}
          {nextVisit ? (
            <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-accent-secondary via-surface-primary to-accent-secondary/30 border border-accent-secondary p-5 shadow-sm">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <p className="text-[11px] font-semibold uppercase tracking-wider text-accent-primary/60">Next visit</p>
                  {getCountdown() && (
                    <p className="mt-1 text-sm font-bold text-accent-primary">{getCountdown()}</p>
                  )}
                  <h2 className="mt-3 text-lg font-bold text-text-primary font-heading leading-snug">
                    {nextVisit.service}
                  </h2>
          ...
          {(pendingBookings.length > 0 || (threadCount !== null && threadCount > 0)) && (
            <div className="rounded-2xl bg-surface-primary shadow-sm p-5">
              <h3 className="text-[11px] font-semibold text-text-tertiary uppercase tracking-wider mb-3">Heads up</h3>
          ...
          {!data.latestReport && (!data.recentBookings || data.recentBookings.length === 0) && data.upcomingCount === 0 && petsCount === 0 && (
            <div className="rounded-2xl bg-accent-tertiary p-8 text-center">
              <h2 className="text-[20px] font-bold text-text-primary font-heading mb-1.5">Welcome to Snout</h2>
              <p className="text-[14px] text-text-secondary mb-5 max-w-[280px] mx-auto leading-relaxed">
                Get started by adding your pets and booking your first visit.
              </p>
  ...
  return (
    ...
    >
      ...
      <div className="p-5">
        ...
        <h3 className="text-[16px] font-semibold text-text-primary">{report.service || 'Update'}</h3>
...
function ClientHomeSkeleton() {
  return (
    <div className="space-y-4 animate-pulse">
      ...
      <div className="grid grid-cols-2 gap-3">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="rounded-2xl border border-border-default bg-surface-primary p-4 min-h-[88px]">
