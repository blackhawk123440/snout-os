...
export async function POST(
  ...
  try {
    ...
    const booking = await (prisma as any).booking.findFirst({
      where: whereOrg(ctx.orgId, { id: bookingId, sitterId: ctx.sitterId }),
    ...
    const incident = await (prisma as any).incidentReport.create({
      data: {
        orgId: ctx.orgId,
        bookingId,
    ...
    // Health log
    if (d.petId) {
      await (prisma as any).petHealthLog.create({
        data: {
          orgId: ctx.orgId,
          petId: d.petId,
          sitterId: ctx.sitterId,
