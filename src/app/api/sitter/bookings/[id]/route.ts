...
export async function GET(
  ...
  try {
    ...
    const latestVisitEvent = await prisma.visitEvent.findFirst({
      where: whereOrg(ctx.orgId, { bookingId: booking.id, sitterId: ctx.sitterId }),
    ...
    const latestReport = await prisma.report.findFirst({
      where: whereOrg(ctx.orgId, { bookingId: booking.id }),
      orderBy: { createdAt: 'desc' },
