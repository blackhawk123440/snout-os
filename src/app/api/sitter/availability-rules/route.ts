...
export async function GET() {
  ...
  try {
    const rules = await (prisma as any).sitterAvailabilityRule.findMany({
      where: whereOrg(ctx.orgId, { sitterId: ctx.sitterId }),
...
export async function POST(request: NextRequest) {
  try {
    ...
    const rule = await (prisma as any).sitterAvailabilityRule.create({
      data: {
        orgId: ctx.orgId,
