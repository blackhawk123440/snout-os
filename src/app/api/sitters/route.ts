  ...
  if (API_BASE_URL) {
    try {
      ...
      const transformedSitters = sitters.map((sitter: any) => ({
        ...
        calendarSyncEnabled: Boolean(sitter.calendarSyncEnabled),
        googleAuthExpired: Boolean(sitter.googleAuthExpired),
        googleAuthError: sitter.googleAuthError || null,
        googleCalendarId: sitter.googleCalendarId || null,
  ...
  try {
    ...
    const transformedSitters = sitters.map((sitter) => {
      ...
      return {
        ...
        calendarSyncEnabled: Boolean((sitter as any).calendarSyncEnabled),
        googleAuthExpired: Boolean((sitter as any).googleAuthExpired),
        googleAuthError: (sitter as any).googleAuthError || null,
        googleCalendarId: (sitter as any).googleCalendarId || null,
        createdAt: sitter.createdAt,
        updatedAt: sitter.updatedAt,
        currentTier: null,
