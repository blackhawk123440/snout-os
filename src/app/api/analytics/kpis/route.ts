...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const revenueToday = (revenueTodayCents._sum?.amount ?? 0) / 100;
    const revenueWeek = (revenueWeekCents._sum?.amount ?? 0) / 100;
    const revenueMonth = (revenueMonthCents._sum?.amount ?? 0) / 100;
    const revenuePeriod = (revenuePeriodCents._sum?.amount ?? 0) / 100;
    const revenuePrev = (revenuePrevCents._sum?.amount ?? 0) / 100;
    ...
    const payoutVolumePeriod = (payoutVolumeCentsPeriod._sum?.amount ?? 0) / 100;
    const payoutVolumePrev = (payoutVolumeCentsPrev._sum?.amount ?? 0) / 100;
