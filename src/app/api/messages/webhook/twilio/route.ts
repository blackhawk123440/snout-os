...
export async function POST(request: NextRequest) {
  try {
    ...
    // Get orgId for logging context (will be resolved after signature verification)
    // Get orgId for logging context
    ...
    // If client doesn't exist, create a minimal one for the thread
    // In production, this might link to an existing client or create a placeholder
    if (!client) {
      // Don't create client automatically - let it be linked later if needed
      // For now, we'll create a thread with clientId null
    }

    // Find or create thread
    // For V1, we'll create a client_general thread if no booking context
