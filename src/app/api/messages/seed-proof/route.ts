...
async function seedMessagingProof() {
  ...
  const client1 = await (prisma as any).client.findFirst({
  ...
  const client2 = await (prisma as any).client.findFirst({
  ...
  const client3 = await (prisma as any).client.findFirst({
  ...
  let frontDeskNumber = await (prisma as any).messageNumber.findFirst({
    where: { orgId: org.id, class: 'front_desk', status: 'active' },
  });

  if (!frontDeskNumber) {
    frontDeskNumber = await (prisma as any).messageNumber.create({
      data: {
        orgId: org.id,
        e164: '+15559876543',
        class: 'front_desk',
  ...
  let poolNumber = await (prisma as any).messageNumber.findFirst({
    where: { orgId: org.id, class: 'pool', status: 'active' },
  });

  if (!poolNumber) {
    poolNumber = await (prisma as any).messageNumber.create({
      data: {
        orgId: org.id,
        e164: '+15559876544',
        class: 'pool',
  ...
  let quarantinedNumber = await (prisma as any).messageNumber.findFirst({
    where: { orgId: org.id, status: 'quarantined' },
  });

  if (!quarantinedNumber) {
    quarantinedNumber = await (prisma as any).messageNumber.create({
      data: {
        orgId: org.id,
        e164: '+15559876545',
        class: 'pool',
  ...
  let unreadThread = await (prisma as any).thread.findFirst({
    where: { orgId: org.id, clientId: client1.id },
  });

  if (!unreadThread) {
    unreadThread = await (prisma as any).thread.create({
      data: {
        orgId: org.id,
        clientId: client1.id,
  ...
  let failedDeliveryThread = await (prisma as any).thread.findFirst({
    where: { orgId: org.id, clientId: client2.id },
  });

  if (!failedDeliveryThread) {
    failedDeliveryThread = await (prisma as any).thread.create({
      data: {
        orgId: org.id,
        clientId: client2.id,
  ...
  const failedMessage = await (prisma as any).message.create({
    data: {
      orgId: org.id,
      threadId: failedDeliveryThread.id,
      direction: 'outbound',
  ...
  let policyViolationThread = await (prisma as any).thread.findFirst({
    where: { orgId: org.id, clientId: client3.id },
  });

  if (!policyViolationThread) {
    policyViolationThread = await (prisma as any).thread.create({
      data: {
        orgId: org.id,
        clientId: client3.id,
        numberId: poolNumber.id,
  ...
  const violationMessage = await (prisma as any).message.create({
    data: {
      orgId: org.id,
      threadId: policyViolationThread.id,
      direction: 'inbound',
  ...
  if (!existingWindow) {
    await (prisma as any).assignmentWindow.create({
      data: {
        orgId: org.id,
        threadId: unreadThread.id,
        sitterId: sitter.id,
