  ...
  describe('Offboarding Integration Hooks', () => {
    it('should deactivate sitter masked number', async () => {
      const mockMaskedNumber = {
        ...
        status: 'active',
        messageNumber: { id: 'number-1', e164: '+15551234568' },
      };

      (prisma.sitterMaskedNumber.findUnique as any).mockResolvedValue(mockMaskedNumber);
      (prisma.sitterMaskedNumber.update as any).mockResolvedValue({
        ...mockMaskedNumber,
        status: 'deactivated',
      ...
      expect(result.maskedNumberId).toBe('masked-1');
      expect(result.messageNumberId).toBe('number-1');
      expect(prisma.sitterMaskedNumber.update).toHaveBeenCalledWith({
        where: { id: 'masked-1' },
        data: {
          status: 'deactivated',
    ...
    it('should reassign sitter threads to owner inbox', async () => {
      ...
      (prisma.threadAssignmentAudit.create as any).mockResolvedValue({ id: 'audit-1' });
      ...
      expect(prisma.threadAssignmentAudit.create).toHaveBeenCalledTimes(2);
    });
  });
});
