 ...
 */
async function detectMissingBookings(orgId: string): Promise<MissingBookingAlert[]> {
  const now = new Date();
  ...
  for (const client of clients) {
    ...
    for (const [dowStr, info] of Object.entries(dayCounts)) {
      ...
      const existingThisWeek = await prisma.booking.findFirst({
        where: {
          orgId,
          clientId: client.id,
          status: { notIn: ['cancelled', 'canceled'] },
 ...
 */
async function forecastDemand(orgId: string): Promise<DemandForecastDay[]> {
  const now = new Date();
 ...
 */
async function projectRevenue(orgId: string): Promise<RevenueProjection> {
  const now = new Date();
...
export async function GET() {
  ...
  try {
    // getScopedDb is available but we use prisma directly with explicit orgId
    // for the helper functions, consistent with the matching engine pattern
    const _db = getScopedDb(ctx);

    const [missingBookingAlerts, demandForecast, revenueProjection] =
      await Promise.all([
        detectMissingBookings(ctx.orgId),
        forecastDemand(ctx.orgId),
        projectRevenue(ctx.orgId),
      ]);
