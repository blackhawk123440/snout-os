...
export default function CalendarPage() {
  ...
  const getBookingsForDate = (date: Date): Booking[] => {
    ...
    return bookings.sort((a, b) => {
      const aTime = booking.timeSlots && booking.timeSlots.length > 0
        ? Math.min(...booking.timeSlots.map((s) => new Date(s.startAt).getTime()))
        : new Date(a.startAt).getTime();
      const bTime = booking.timeSlots && booking.timeSlots.length > 0
        ? Math.min(...booking.timeSlots.map((s) => new Date(s.startAt).getTime()))
