...
function KpiCard({ item }: { item: KpiItem }) {
  const content = (
    <div
      className={cn(
        'rounded-xl border border-[var(--color-border-default)] bg-[var(--color-surface-primary)] p-4 shadow-sm transition',
        item.href
          ? 'cursor-pointer hover:border-[var(--color-border-muted)] hover:shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-accent-primary)] focus-visible:ring-offset-1'
          : ''
      )}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 truncate text-2xl font-bold text-[var(--color-text-primary)] tabular-nums">{item.value}</p>
        ...
        {item.icon && (
          <div className="shrink-0 text-2xl text-[var(--color-text-tertiary)]">{item.icon}</div>
        )}
      </div>
    </div>
  );
