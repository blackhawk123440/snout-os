...
export function NumbersPageContent() {
  ...
  const handleQuarantine = async () => {
    ...
    try {
      const result = await quarantineNumber.mutateAsync({
        ...
        durationDays: quarantineForm.durationDays,
        customReleaseDate: quarantineForm.customReleaseDate,
      });
      alert(`Number quarantined. Impact: ${result.impact.message}`);
      setShowQuarantineModal(null);
      setQuarantineForm({ reason: '', reasonDetail: '', durationDays: 90, customReleaseDate: undefined });
    } catch (error: any) {
          ...
          <Modal isOpen={!!showQuarantineModal} title="Quarantine Number" onClose={() => setShowQuarantineModal(null)}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: tokens.spacing[4] }}>
              ...
              <div>
                ...
                <select
                  value={quarantineForm.durationDays?.toString() || '90'}
                  onChange={(e) => {
                    const value = e.target.value;
                    if (value === 'custom') {
                      setQuarantineForm({ ...quarantineForm, durationDays: undefined, customReleaseDate: undefined });
                    } else {
              ...
              {quarantineForm.durationDays === undefined && (
                <div>
                  ...
                  <Input
                    type="date"
                    value={quarantineForm.customReleaseDate || ''}
                    onChange={(e) => setQuarantineForm({ ...quarantineForm, customReleaseDate: e.target.value })}
