...
export const InteractiveRow = forwardRef<HTMLDivElement, InteractiveRowProps>(
  ({ children, className, as: As = 'div', ...props }, ref) => {
    const base =
      'cursor-pointer transition-colors duration-150 focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-accent-primary)] focus-visible:ring-offset-1 rounded-md';
    const hover = 'hover:bg-[var(--color-surface-tertiary)]';
    const active = 'active:bg-[var(--color-surface-secondary)]';

    if (As === 'tr') {
      return (
        <tr
          ref={ref as React.Ref<HTMLTableRowElement>}
          className={cn(base, hover, active, 'border-b border-[var(--color-border-default)]', className)}
